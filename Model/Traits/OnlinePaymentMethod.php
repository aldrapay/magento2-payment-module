...
trait OnlinePaymentMethod
{
    ...
    protected function processReferenceTransaction(
        ...
        $aldrapay = "\\Aldrapay\\$transactionType";
        $aldrapay = new $aldrapay;

        $aldrapay->setParentUid($data['reference_id']);
        $aldrapay->money->setAmount($data['amount']);
        $aldrapay->money->setCurrency($data['currency']);

        if (strtolower($transactionType) == $helper::REFUND)
          $aldrapay->setReason($data['reason']);

        $responseObject = $aldrapay->submit();
